...
export default class Puzzle2Scene extends Phaser.Scene {
    ...
    preload() {
       this.load.image('s_full_image', 'assets/conjunto-da-basilica-do-senhor-bom-jesus-de-matosinhos.png');
    }
    ...
    create() {
        ...
        const gabarito = this.add.image(targetX, targetY, 's_full_image');
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                const frameX = col * (this.textures.get('s_full_image').source[0].width / cols);
                const frameY = row * (this.textures.get('s_full_image').source[0].height / rows);

                // Cria uma nova textura temporária para cada peça
                // Isso "corta" um pedaço da imagem original
                const texture = this.textures.get('s_full_image');
                texture.add(
                    `piece_${row}_${col}`, 
                ...
                const piece = this.add.image(
                    ...
                    's_full_image', // Usa a textura original...
