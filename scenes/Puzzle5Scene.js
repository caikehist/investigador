...
export default class Puzzle5Scene extends Phaser.Scene {
    ...
    preload() {
       this.load.image('full_image_5', 'assets/centro-historico-de-paraty.png');
    }
    ...
    create() {
        ...
        this.add.text(width / 2, 50, 'Patrimônio 5: Monte o Quebra-Cabeça\nCentro Histórico de Paraty - ', {
            fontSize: '28px',
            fill: '#FFF'
        }).setOrigin(0.5);
        ...
        const gabarito = this.add.image(targetX, targetY, 'full_image_5');
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                const frameX = col * (this.textures.get('full_image_5').source[0].width / cols);
                const frameY = row * (this.textures.get('full_image_5').source[0].height / rows);

                // Cria uma nova textura temporária para cada peça
                // Isso "corta" um pedaço da imagem original
                const texture = this.textures.get('full_image_5');
                texture.add(
                    `piece_${row}_${col}`, 
                ...
                const piece = this.add.image(
                    ...
                    'full_image_5', // Usa a textura original...
    ...
    checkWinCondition() {
        ...
        if (allPlaced) {
            ...
            this.game.events.emit('updateScore', 25)

            // 2. SALVA O PROGRESSO: Registra que a próxima cena é a FaseFinalScene
            localStorage.setItem('phaser_saved_stage', 'Transition1Scene'); // << NOVO CÓDIGO AQUI
            // Aqui você chamaria a próxima cena/minigame:
            this.time.delayedCall(2000, () => this.scene.start('Transition1Scene'));
        }
    }
