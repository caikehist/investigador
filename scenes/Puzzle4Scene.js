...
export default class Puzzle4Scene extends Phaser.Scene {
    ...
    preload() {
       this.load.image('full_image_4', 'assets/ruinas-de-sao-miguel-das-missoes.png');
    }
    ...
    create() {
        ...
        this.add.text(width / 2, 50, 'Patrimônio 4: Monte o Quebra-Cabeça\nRuínas de São Miguel das Missões', {
            fontSize: '28px',
            fill: '#FFF'
        }).setOrigin(0.5);
        ...
        const gabarito = this.add.image(targetX, targetY, 'full_image_4');
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                const frameX = col * (this.textures.get('full_image_4').source[0].width / cols);
                const frameY = row * (this.textures.get('full_image_4').source[0].height / rows);

                // Cria uma nova textura temporária para cada peça
                // Isso "corta" um pedaço da imagem original
                const texture = this.textures.get('full_image_4');
                texture.add(
                    `piece_${row}_${col}`, 
                ...
                const piece = this.add.image(
                    ...
                    'full_image_4', // Usa a textura original...
    ...
    checkWinCondition() {
        ...
        if (allPlaced) {
            ...
            localStorage.setItem('phaser_saved_stage', 'Puzzle5Scene'); // << NOVO CÓDIGO AQUI
            // Aqui você chamaria a próxima cena/minigame:
            this.time.delayedCall(2000, () => this.scene.start('Puzzle5Scene'));
        }
    }
